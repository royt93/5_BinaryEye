...
| BUG-18 | 🟠 P1 — Tính năng hỏng | **`ActivityPick` ("Pick file") không decode được ảnh mã vạch hợp lệ.** Tự phát hiện khi live-test FEAT-NEW-01 (VietQR): tạo QR bằng chính app (Encode → PNG 640px và 1024px), lưu Downloads, mở lại qua "Pick file" → bấm "Scan" → **"Không tìm thấy mã vạch"** dù ảnh hợp lệ 100% (verify bằng mắt + payload đúng). **Tái hiện trên 2 thiết bị khác nhau, 2 Android version khác nhau** (Galaxy A50s Android 11, Galaxy S24 Ultra Android 16) — không phải lỗi thiết bị/độ phân giải. Đã loại trừ: format restriction (`prefs.barcodeFormats` mặc định có QR_CODE), crop-handle position tồn dư (`prefs.cropHandleX/Y = -2`, tức "mặc định"), `am start` reuse-instance stale (đã force-stop + fresh launch, vẫn fail), native ZXing lib load lỗi (log xác nhận `libzxingcpp_android.so` load + gọi decode thành công, không exception). **Nghi vấn root cause:** `scanWithinBounds()` (dòng 127-145) khi chưa có ROI tùy chỉnh (`detectorView.roi.width() < 1`) fallback dùng `cropImageView.getBoundsRect()` làm vùng crop để decode — quan sát trực quan overlay tô xám (vùng sẽ decode) KHÔNG che phủ trọn vẹn ảnh, có dải mỏng ở cạnh phải/dưới nằm ngoài vùng tô xám → nếu đúng, vùng đó cũng bị loại khỏi crop-rect dùng decode, có thể cắt mất module QR quan trọng (finder pattern/timing pattern) tùy tỷ lệ ảnh. Chưa xác định chính xác 100% vì cần debug `ScalingImageView` (thư viện ngoài `com.github.markusfisch:ScalingImageView`) bằng breakpoint thật trong Android Studio, không làm được qua ADB screen-scraping. **Ảnh hưởng:** mọi user dùng "Pick file" để quét ảnh chụp/lưu sẵn có thể gặp lỗi này tùy tỷ lệ khung hình — không giới hạn ở VietQR/tính năng mới nào, đây là hạ tầng cũ đã có từ trước. | `view/act/ActivityPick.kt:127-201`, `view/widget/CropImageView.kt` | Tự phát hiện qua device-test session 2026-08-22 (chưa qua review AI) | Debug `cropImageView.getBoundsRect()`/`mappedRect` bằng breakpoint thật để xác nhận có lệch với kích thước ảnh hiển thị thật không; nếu đúng, sửa bằng cách dùng full bitmap rect (`Rect(0,0,bitmap.width,bitmap.height)`) làm mặc định thay vì `getBoundsRect()` khi chưa có ROI tùy chỉnh. |
...
| 🟡 FEAT-NEW-01 ⭐🎯 | **VietQR / EMVCo Banking QR Parser** | 🟡 **CODE DONE 2026-08-21, unit-test đầy đủ; device round-trip qua "Pick file" bị chặn bởi BUG-18 (bug pre-existing, không liên quan)** — parser EMVCo TLV + VietQR/NAPAS thuần (`EmvQrParser`/`VietQrParser`), tách Ngân hàng/Số tài khoản/Số tiền/Nội dung, copy nhanh số tài khoản, hiển thị đẹp thay blob thô. Mở app ngân hàng cụ thể ngoài scope (không có chuẩn chung). 11 unit test pass (payload mẫu dựng đúng chuẩn thật, theo cấu trúc TLV EMVCo/NAPAS công khai). **Root cause đã xác định 2026-08-22** (tái hiện trên 2 thiết bị/Android version khác nhau): lỗi nằm ở `ActivityPick` (hạ tầng "Pick file" dùng chung, xem BUG-18), không phải VietQR — live camera decode không bị ảnh hưởng. Xem `doc/task/done/FEATNEW01-vietqr-parser-mvp.md`. | ⚠️ 1 nguồn (agy) nhưng đánh giá **"Business Value: Rất cao, Độ khó: Thấp-Vừa"**, rất hợp thị trường VN/ĐNA của app này | **Đề xuất flagship mới**, xem Q3 AskUserQuestion |
