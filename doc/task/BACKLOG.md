...
| 🟡 F10 | ~~OCR → QR~~ → **OCR → VCARD** | 🟡 **CODE DONE 2026-08-22, device verify chưa làm** — `BusinessCardParser.kt` (pure, tách tên/SĐT/email/công ty/chức danh từ text OCR) + `ActivityOcrCard.kt` (ML Kit Text Recognition on-device + dialog sửa tay + tái dùng `VCardAction` có sẵn để thêm Danh bạ). Menu "Scan business card (OCR)" trong overflow camera. 8 unit test pass. Device test bị chặn do thiết bị mất kết nối USB (không liên quan code). Xem `doc/task/done/F10-ocr-vcard.md`. | 🎯 **CHỐT: re-scope theo khuyến nghị AI** |
| 🟡 FEAT-NEW-01 ⭐🎯 | **VietQR / EMVCo Banking QR Parser** | 🟡 **CODE DONE 2026-08-21, device round-trip chưa verify xong** — parser EMVCo TLV + VietQR/NAPAS thuần (`EmvQrParser`/`VietQrParser`), tách Ngân hàng/Số tài khoản/Số tiền/Nội dung, copy nhanh số tài khoản, hiển thị đẹp thay blob thô. Mở app ngân hàng cụ thể ngoài scope (không có chuẩn chung). 11 unit test pass (payload mẫu dựng đúng chuẩn thật). Device test bị chặn bởi lỗi "No barcode found" khi decode ảnh QR tĩnh qua Pick-from-file (chưa rõ nguyên nhân, không liên quan code VietQR) + thiết bị mất kết nối giữa chừng. Xem `doc/task/done/FEATNEW01-vietqr-parser-mvp.md`. | ⚠️ 1 nguồn (agy) nhưng đánh giá **"Business Value: Rất cao, Độ khó: Thấp-Vừa"**, rất hợp thị trường VN/ĐNA của app này | **Đề xuất flagship mới**, xem Q3 AskUserQuestion |
