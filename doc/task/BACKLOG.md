...
**Sprint 0 — ✅ DONE (2026-08-21)** — cả 8 item đã code + `compileDevDebugKotlin` BUILD SUCCESSFUL + `testDevDebugUnitTest` 33 test pass. File đã chuyển sang `doc/task/done/`:
...
Lint (`lintDevDebug`) fail nhưng lỗi (`preferences.xml` — "entity name must immediately follow '&'") đã **verify pre-existing bằng git stash trước khi có Sprint 0** — không liên quan 8 fix trên, ngoài scope, chưa xử lý.

**Sprint 1 — ✅ DONE (2026-08-21)** — 10/11 item đã code (BUG-07,08,09,10,11,12,13,14,15,17) + build/test pass. BUG-16 **chủ động không sửa** — `git log` cho thấy mismatch versionCode "2027xxxx" vs versionName "2026.xx.xx" là convention nhất quán qua nhiều release, không phải typo; sửa sẽ phá tính đơn điệu tăng của versionCode trên Play Store, cần dev xác nhận trước. Verify trên Pixel 7 Pro: BUG-10 (CSV export) confirm đúng 12 cột header/data khớp nhau (trước fix data thừa 1 cột); BUG-11/12 (History search/cursor) không crash qua nhiều lượt search/xoá. Chi tiết từng item xem `doc/task/done/BUG-07*.md` → `BUG-17*.md`.

**Sprint 1 — 🎯 CHỐT với user (2026-08-21):** BUG-07 → BUG-17 (11 item P2/P3). Thứ tự VIP feature sau này (khi bắt đầu Epic F): VIP-01 Secure Vault trước, rồi VIP-02/VietQR quyết sau.
...
| ✅ BUG-16 | 🟢 P3 | `versionCode 20270822` (mã hoá năm **2027**) nhưng `versionName '2026.08.22'` (năm **2026**) — lệch 1 năm. **Đã confirm với dev (2026-08-22): bản này ĐÃ PUBLISH lên Play Store** với versionCode 20270822 → không thể giảm versionCode (Play Store bắt buộc tăng dần). Fix: đổi `versionName` → `'2027.08.22'` để khớp năm với versionCode đã publish. | `app/build.gradle:63-64` | ✅ (claude, đã tự verify literal trong build.gradle) | ✅ **DONE 2026-08-22** — sync versionName lên 2027.08.22. |
...
| ✅ BUG-18 | 🟠 P1 — Tính năng hỏng | **`ActivityPick` ("Pick file") không decode được ảnh mã vạch hợp lệ.** ✅ **FIXED + live-verify 2026-08-22.** Root cause thật (đào sâu bằng debug log trực tiếp trên device, không phải suy đoán): `DetectorView.onLayout()` **tự động** đặt 1 vùng crop "gợi ý" (80% từ tâm qua `setHandleToDefaultRoi()`) và set `handleActive = true` **ngay khi ảnh vừa load xong, trước cả khi user chạm màn hình** — `detectorView.roi` vì vậy KHÔNG BAO GIỜ rỗng/width<1 trong thực tế (giả thuyết ban đầu sai), khiến `scanWithinBounds()` trong `ActivityPick` luôn dùng vùng crop gợi ý nhỏ hơn ảnh thật để decode, cắt mất mép barcode (đặc biệt barcode chiếm gần hết khung như QR tự generate). **Fix:** thêm `DetectorView.hasUserAdjustedRoi` (chỉ `true` khi user THẬT SỰ chạm/kéo crop handle qua `onTouchEvent` ACTION_UP, không bị set bởi auto-default) — `ActivityPick.scanWithinBounds()` dùng cờ này thay vì chỉ check `roi.width() < 1`, mặc định quét toàn bộ ảnh (`mappedRect`) cho tới khi user chủ động chỉnh crop. **Live-verify trên Galaxy S24 Ultra sau fix:** QR đơn giản ("hello world test") decode đúng 100% qua Pick file (trước fix: "Không tìm thấy mã vạch"); QR VietQR (149 ký tự, dày đặc) cũng decode đúng, `VietQrAction` hiển thị đúng Bank/Account/Amount/Message + copy account number hoạt động — xác nhận đồng thời cả BUG-18 và FEAT-NEW-01. 106/106 unit test pass. | `view/act/ActivityPick.kt`, `view/widget/DetectorView.kt` | Tự phát hiện + tự fix qua device-test session 2026-08-22 (chưa qua review AI) | Đã fix, xem `doc/task/done/FEATNEW01-vietqr-parser-mvp.md` để biết chi tiết live-verify. |
...
|----|--------|------|-------|---------|
| SEC-01 | **VIP hoàn toàn client-side, crackable**: `VIP_KEY_SECRET` + 2 redeem code (30 ngày/3 ngày) nhúng thẳng vào `BuildConfig` (string literal, R8 không obfuscate được string constant) — decompile APK bằng jadx vài giây là lấy được secret, patch VIP miễn phí vĩnh viễn cho mọi máy. **✅✅✅ Xác nhận thực chiến 2026-08-21**: khi test F8 trên Pixel 7 Pro, đã kích hoạt VIP Active thành công chỉ bằng cách set 1 giá trị `long` (`keyVipByKeyUntil`) qua `run-as` vào `shared_prefs/loitp_admob.xml` của thư viện wrapper — không cần key thật, không cần mạng, không cần decompile, < 5 phút. Xác nhận phiên bản wrapper 1.1.3 đang chạy trong app **không có signature/tamper check** trên giá trị này (dù bản 1.6.16 mới hơn của cùng thư viện — xem `VipSecurity.kt`/`KeystoreIntegrity.kt` trong sources — đã có cơ chế ký, nhưng app chưa upgrade lên). | `app/build.gradle:73` (`VIP_KEY_SECRET`), `frm/FVipManagement.kt:90,439` | ✅✅✅ (claude + codex + tự verify thực chiến) | Đã biết trước 1 phần (doc/AD.MD ghi "đổi secret invalidates key cũ") nhưng chưa từng được flag như **lỗ hổng bảo mật/doanh thu**, nay đã tự tay khai thác thành công. Fix triệt để cần backend verification (Play Integrity API hoặc server redeem) — cộng hưởng tốt với M1 (Play Billing). **Quick win có thể cân nhắc ngay**: upgrade `AdmobApplovinWrapper` 1.1.3 → 1.6.16+ để tận dụng cơ chế ký sẵn có trong thư viện (giảm rủi ro đáng kể mà không cần tự xây backend), cần test kỹ vì cách nhau nhiều minor version. |
...
| ✅ SEC-03 | `backup_rules.xml` include toàn bộ root (cả lịch sử scan chứa nội dung nhạy cảm: WiFi password, thẻ tín dụng quét được, vCard) vào cloud backup mà không có exclude rule rõ ràng. | `app/src/main/res/xml/backup_rules.xml:1` | ✅ (codex) | ✅ **DONE 2026-08-21** — thêm `<exclude domain="database">` vào cả `backup_rules.xml` (legacy) và `extraction_rules.xml` (API 12+). |
...
|----|---------|------------|------------------------|
| ✅ F3 | Scan Groups/Tags | ✅ **DONE 2026-08-22** — DB `tags` column (CSV, v7→v8 migration), `TagUtils.parseTags/joinTags` (pure, unit test), `ScanFilter.tag` (LIKE `,tag,` wrapped, tránh match nhầm substring). UI: menu "Manage tags" trong ActionMode (chỉ 1 scan/lần, giống `showScanAsQr`) — dialog 4 preset checkbox (Work/Personal/Shopping/Travel) + custom tag (VIP-only, gate qua `AdManager.isVipByKeyActive()`). Filter chip row mới `chipGroupTag` (All + 4 preset) trong `roy_f_history.xml`. **Scope-down so với đề xuất gốc**: dùng CSV 1 cột thay vì bảng `tags` + junction table riêng (1 nguồn AI đề xuất) — đơn giản hơn, đủ cho khối lượng dữ liệu 1 app cá nhân, tránh join phức tạp. 10 unit test mới (`TagUtilsTest` + 3 case `ScanFilterTest`), 125/125 test pass, `compileDevDebugKotlin` OK. **Live-verify trên Galaxy S24 Ultra (2026-08-22)**: cài `installDevDebug`, migration DB v7→v8 chạy sạch không crash trên app thật đang có sẵn dữ liệu (`.schema scans` xác nhận cột `tags TEXT` xuất hiện đúng). Seed 2 scan test qua `run-as`+`sqlite3`, gán tag "Work"+"Shopping" qua dialog thật (đúng UI, VIP-gate custom tag field bị disable với hint đúng cho free user) → DB ghi đúng CSV "Work,Shopping". Filter chip "Work" hiện đúng cả 2 scan, "Shopping" chỉ hiện đúng 1 scan (không match nhầm substring), "Personal" trả về rỗng không crash. Đã dọn sạch data test khỏi device sau khi verify. | 1 nguồn khuyên: dùng bảng `tags` + junction table thay vì CSV trong 1 cột (chuẩn hoá hơn) — cân nhắc effort tăng thêm |
| F5 | Barcode compare | 🟡 hỗn hợp: 1 nguồn đồng ý, 2 nguồn hạ ưu tiên (niche use-case) | Hạ xuống 💭 Idea, làm sau |
| ✅ F8 | Biometric lock history | ✅✅✅ đồng ý mạnh — "tính năng đáng giá nhất để thúc đẩy VIP" | ✅ **DONE 2026-08-21** (VIP-01 scope "vừa phải") — `androidx.biometric` BiometricPrompt + DEVICE_CREDENTIAL fallback, gate `AdManager.isVipByKeyActive()`, re-auth mỗi lần rời màn hình. Xem `doc/task/done/VIP01-secure-vault-moderate.md`. |
...
| ✅ F10 | ~~OCR → QR~~ → **OCR → VCARD** | ✅ **DONE 2026-08-22, live-verify thành công end-to-end** — `BusinessCardParser.kt` (pure, tách tên/SĐT/email/công ty/chức danh từ text OCR) + `ActivityOcrCard.kt` (ML Kit Text Recognition on-device + dialog sửa tay + tái dùng `VCardAction` có sẵn để thêm Danh bạ). Menu "Scan business card (OCR)" trong overflow camera. 8 unit test pass. Verify sống trên Galaxy S24 Ultra qua đúng luồng UI thật: OCR chạy đúng, Company/Title/Phone tách đúng, "ADD TO CONTACTS" mở đúng app-chọn Danh bạ, form Contacts thật điền đúng 100%. Xem `doc/task/done/F10-ocr-vcard.md`. | 🎯 **CHỐT: re-scope theo khuyến nghị AI** |
| ✅ FEAT-NEW-01 ⭐🎯 | **VietQR / EMVCo Banking QR Parser** | ✅ **DONE 2026-08-22, live-verify thành công end-to-end** — parser EMVCo TLV + VietQR/NAPAS thuần (`EmvQrParser`/`VietQrParser`), tách Ngân hàng/Số tài khoản/Số tiền/Nội dung, copy nhanh số tài khoản, hiển thị đẹp thay blob thô. Mở app ngân hàng cụ thể ngoài scope (không có chuẩn chung). 11 unit test pass (payload mẫu dựng theo đúng cấu trúc TLV EMVCo/NAPAS công khai). Sau khi fix BUG-18 (bug pre-existing ở `ActivityPick`, chặn Pick file chứ không phải lỗi VietQR), live-verify trên Galaxy S24 Ultra qua Pick file: decode đúng payload VietQR mẫu, `VietQrAction` hiển thị đúng **"Bank: Vietcombank / Account number: 0123456789 / Amount: 500000 VND / Message: Thanh toan don hang"**, nút "COPY ACCOUNT NUMBER" copy đúng + toast xác nhận đúng. Xem `doc/task/done/FEATNEW01-vietqr-parser-mvp.md`. | ⚠️ 1 nguồn (agy) nhưng đánh giá **"Business Value: Rất cao, Độ khó: Thấp-Vừa"**, rất hợp thị trường VN/ĐNA của app này | **Đề xuất flagship mới**, xem Q3 AskUserQuestion |
...
| ✅ FEAT-NEW-05 | **Pagination cho History (hiện load hết 1 lần)** | ✅ **DONE 2026-08-22, live-verify thành công** — `Db.getScans()` thêm tham số `limit` tùy chọn (LIMIT trong SQL, mặc định null = giữ nguyên hành vi cũ cho các call site khác vd export). `FHistory` tải trang đầu 100 dòng, cuộn gần đáy (`HistoryPaging.shouldLoadMore()`, pure function có test) tự tải thêm 100 dòng tiếp, giữ nguyên vị trí cuộn (`setSelectionFromTop`) thay vì nhảy về đầu. Reset về trang đầu khi đổi filter/search/rename/remove (giữ nguyên hành vi cũ cho các thao tác đó). 9 unit test cho `HistoryPaging` (pure, không phụ thuộc Android). Live-verify trên Galaxy S24 Ultra: seed 150 dòng test qua sqlite3, cuộn từ dòng 150 xuống tận dòng 1 mượt mà, không crash, không bị chặn ở mốc 100 như trước — xác nhận vượt qua giới hạn trang đầu đúng thiết kế. 115/115 unit/widget test pass toàn project. | Effort thấp-trung bình, ảnh hưởng hiệu năng thực tế khi lịch sử lớn |
...
| ✅ VIP-02 ⭐🎯 | **B2B Batch Audit & Inventory Suite (MVP)** | ✅ **DONE 2026-08-21, live-verify qua camera thật** — MVP: quét liên tục đếm số lượng, so khớp expected-list, tone riêng biệt cho OK/DUPLICATE/UNEXPECTED, badge live-count, bottom sheet tổng kết, export CSV chuẩn cột. Sau khi ship, tự audit + smoke test thật đã tìm và fix 4 bug thật (silent-mode beep, mất dữ liệu khi xoay máy, quét lại 1 mã bị nuốt vĩnh viễn, `decoding` bị khoá nếu đã quét thường trước đó). **Chưa làm** (deferred, không nằm trong MVP): GS1 AI parser (lot/expiry/serial), cảnh báo hàng giả/trùng serial *xuyên phiên* (MVP chỉ dedupe trong 1 session). Xem `doc/task/done/VIP02-batch-audit-mvp.md`. | ✅✅✅✅ **CẢ 4 NGUỒN** đều đề xuất biến thể của ý này độc lập (agy: multi-barcode inventory; claude: batch business workflow + verify&track; codex: batch audit session + GS1 inspector + inventory mode) — tín hiệu mạnh NHẤT toàn review, nhắm thị trường B2B/SME sẵn sàng trả tiền hơn cá nhân | L (MVP: M, GS1+cross-session deferred) |
